...
class Queue(object):
    ...
    def from_queue_key(cls, queue_key, connection=None):
        ...
        name = queue_key[len(prefix):]  # 去掉`rq:queue:default`中的`rq:queue:`
        return cls(name, connection=connection)  # 返回该类的实例，cls代表Queue

    def __init__(self, name='default', default_timeout=None, connection=None,
                 async=True, job_class=None):
        self.connection = resolve_connection(connection)
        prefix = self.redis_queue_namespace_prefix
        self.name = name  # 默认队列名：default
        self._key = '{0}{1}'.format(prefix, name)
        self._default_timeout = default_timeout
        self._async = async  # 默认为异步
    ...
    # 清除job队列中死掉的job（job队列中只是保存job的key，如果job的key不在redis中，认为该job挂掉了）
    def compact(self):
    ...
    def enqueue_call(self, func, args=None, kwargs=None, timeout=None,
        ...
        if depends_on is not None:   # 该job有依赖
            ...
            with self.connection._pipeline() as pipe:
                while True:
                    try:
                        pipe.watch(depends_on.key)  # 观察depends_on.key是否修改，若修改，抛出错误。主要用在redis的事务里。
                        if depends_on.get_status() != JobStatus.FINISHED:  # 依赖的job未完成
                            pipe.multi()
                            job.set_status(JobStatus.DEFERRED)
                            job.register_dependency(pipeline=pipe)  # 让被依赖的job P保存本job C的信息，当被依赖的job P处理完后，再处理本job C。
                            job.save(pipeline=pipe)  # job的具体信息放入redis中，为什么不放入job队列？？？ 原因在`job.register_dependency`中
                            pipe.execute()
                            return job
                        break  # 若依赖的job完成了，则进入下下下行继续执行
                    except WatchError:
                        continue

        job = self.enqueue_job(job, at_front=at_front)  # 保存job具体信息，若是异步任务，则放入job队列

        if not self._async:   # 非异步任务
            job.perform()    # 直接执行
            job.save()       # 仍然保存（执行完的任务保留下来很有必要）
    ...
    def enqueue(self, f, *args, **kwargs):
        ...
        * A string, representing the location of a function (must be
        ...
        print '>> queue.py: Queue.enqueue:', f.__name__, args, kwargs

        if not isinstance(f, string_types) and f.__module__ == '__main__':  # 只能将其他模块中定义的任务
        ...
        job_id = kwargs.pop('job_id', None)enqueue_jobenqueue_jobpusj
    ...
    def enqueue_job(self, job, pipeline=None, at_front=False):
        ...
        job.save(pipeline=pipe)   # 再redis中保存job的具体信息

        if pipeline is None:
            pipe.execute()

        if self._async:  # 若为异步，则将job id保存到job队列中
