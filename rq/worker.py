...
class Worker(object):
    ...
    def find_by_key(cls, worker_key, connection=None):
        ...
        if connection is None:
            connection = get_current_connection()
        if not connection.exists(worker_key):  # 若拿不到该worker的具体信息
            connection.srem(cls.redis_workers_keys, worker_key)  # 从worker集合中移除该worker
            return None

        name = worker_key[len(prefix):]  # 提取worker的name
    ...
    def __init__(self, queues, name=None,
        ...
        if exception_handlers is None:
            self.push_exc_handler(self.move_to_failed_queue)  # execption handler，如果job执行出问题，则将job移到失败队列
        ...
        try:
            while True:
                try:
                    ...
                    result = self.dequeue_job_and_maintain_ttl(timeout)  # 出队一个job（worker主动拉取job）
                    if result is None:
                        if burst:
                ...
                self.heartbeat()    # 更新redis中该worker的过期时间
    ...
    def execute_job(self, job):
        ...
        if child_pid == 0:  # 子进程
            ...
            while True:
                try:
                    self.set_state('busy')
                    os.waitpid(child_pid, 0)  # 等待子进程完成
                    self.set_state('idle')
                    break
                except OSError as e:
